...
public class StoreUpd extends JFrame implements ActionListener {
	...
	{
		setLayout(null);
		nu5=new JLabel("Enter Employee Number");
		 ...
		 if(ae.getSource()==b)
		 {
			 String StoreId=tf1.getText();
				...
				try {
					Class.forName("oracle.jdbc.driver.OracleDriver");
					con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
					String str="update Store set name=?,address=?,gstin=?,phone=?,email=?,pan=? where StoreId=?";
					st=con.prepareStatement(str);
					st.setString(1, name);
