...
public class ItemsUpd extends JFrame implements ActionListener {
	...
	public ItemsUpd()
	{
		...
		nu6.setBounds(100, 520, 400, 40);
		nu6.setFont(new Font("Calibri",Font.BOLD,24));
		add(nu6);
		tf6=new JTextField();
		tf6.setBounds(550, 520, 200, 40);
		add(tf6);
		b=new JButton("Update");
		b.setBounds(100, 600 , 120, 60);
		b.setFont(new Font("Arial",Font.BOLD,20));
		add(b);
		b.addActionListener(this);
		e=new JButton("Exit");
		e.setBounds(300,600 , 120, 60);
		 ...
		 if (ae.getSource()==r)
		 {
			 String e=tf1.getText();
				try {
					...
					String str="select itemcatid,itemname,pricepunit,openq,currq from itemsn where itemid=?";
		 ...
		 if(ae.getSource()==b)
		 {
			 String StoreId=tf1.getText();
				...
				try {
					Class.forName("oracle.jdbc.driver.OracleDriver");
					con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","system","manager");
					String str="update itemsn set itemcatid=?,itemname=?,pricepunit=?,openq=?,currq=? where itemid=?";
					st=con.prepareStatement(str);
					st.setString(1, name);
